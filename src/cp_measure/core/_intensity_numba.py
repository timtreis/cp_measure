...
def _compute_intensity_features_2d(masks, pixels):
    ...
    # Pass 1: count pixels and edge pixels per label, mark edge map
    ...
    # Compute prefix-sum offsets for flat per-label arrays (two-pass pattern)
    ...
    # Pass 2: fill flat arrays with pixel values and coordinates
    for y in range(height):
        for x in range(width):
            label = np.int64(masks[y, x])
            if label > 0:
